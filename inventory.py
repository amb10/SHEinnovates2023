    ...
    if request.method == "POST":
        ...
        if request.form.__contains__("Delete"):
            ...
            for item in masterInventory:
                if item.name == request.form.get("Delete"):
                    deleted = True
                    if (int(quantity) >= int(item.inventory)):
                        print(quantity)
                        print(item.inventory)
                        print("remove all")
                        masterInventory.remove(item)
                    elif (int(quantity) < int(item.inventory) and int(quantity) > 0):
                        print("remove some")
                        item.inventory = int(item.inventory) - int(quantity)
                    elif(int(quantity) < 0):
        ...
        # Add an item to the inventory
            ...
            newItem = FoodItem(request.form.get("Name"), None)
            newItem.expiration = [request.form.get("Month"), request.form.get("Day"), request.form.get("Year")]
            newItem.inventory = request.form.get("Quantity")
            newItem.glutenFree = request.form.get("GF")
            newItem.dairyFree = request.form.get("DF")
            newItem.nutFree = request.form.get("NF")
            newItem.vegan = request.form.get("Vegan")
            newItem.vegetarian = request.form.get("Vegetarian")
            newItem.expired = updateExpiration(newItem.expiration, today)
...
def sortName():
    ...
    masterInventory.sort(key=lambda x: x.name.lower())
    debugPrint()

def sortDateDescending():
    print("Sorting by date descending")
    masterInventory.sort(key=lambda x: x.expiration[0], reverse=True)
    masterInventory.sort(key=lambda x: x.expiration[1], reverse=True)
    masterInventory.sort(key=lambda x: x.expiration[2], reverse=True)
    debugPrint()

def sortDateAscending():
    print("Sorting by date ascending")
    masterInventory.sort(key=lambda x: x.expiration[0], reverse=False)
    masterInventory.sort(key=lambda x: x.expiration[1], reverse=False)
    masterInventory.sort(key=lambda x: x.expiration[2], reverse=False)
    debugPrint()
